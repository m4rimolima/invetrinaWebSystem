...
<div class="container mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4">Adicionar Exposição</h1>
    ...
    <form action="{{ route('exposicoes.store') }}" method="POST" class="bg-white p-6 rounded shadow-md">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="local" id="local" value="{{ old('local') }}" class="w-full border border-gray-300 rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label for="data_inicio" class="block text-gray-700 font-medium mb-2">Data Início</label>
            <input type="date" name="data_inicio" id="data_inicio" value="{{ old('data_inicio') }}" class="w-full border border-gray-300 rounded px-3 py-2">
        </div>

        <div class="mb-4">
            <label for="data_fim" class="block text-gray-700 font-medium mb-2">Data Fim</label>
            <input type="date" name="data_fim" id="data_fim" value="{{ old('data_fim') }}" class="w-full border border-gray-300 rounded px-3 py-2">
        </div>
